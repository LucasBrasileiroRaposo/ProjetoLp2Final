...
public class RepositorioPesquisador implements Busca {
    ...
    private RepositorioPesquisador controladorPesquisador;

    /**
     * Construtor responsável por inicializar o armazenamento de pesquisadores
     */
    public RepositorioPesquisador() {
        this.mapaDePesquisadores = new HashMap<>();
        this.controladorPesquisador = new RepositorioPesquisador();
    }
    ...
    public int contaResultadosBusca(String termo) {
        Validadora.verificaValorNullVazio(termo,"Campo termo nao pode ser nulo ou vazio.");
        int cont = 0;
        for(String palavra: controladorPesquisador.busca(termo).split(" | ")){
    ...
    public String busca(String termo, int numeroDoResultado) {
        Validadora.verificaValorNullVazio(termo,"Campo termo nao pode ser nulo ou vazio.");
        int cont = 0;
        if(termo.contains(controladorPesquisador.busca(termo))){
            for(String palavra: controladorPesquisador.busca(termo).split(" | ")){
                cont +=1;
